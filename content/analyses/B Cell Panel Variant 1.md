...
>
> **★ [2026-08-28] "EF" in the panel names below is legacy shorthand, not a location claim.** A twelve-author consensus Perspective holds that *"there are no flow cytometry-based means alone that can distinguish EF B cells nor their progeny,"* and that of this exact cell, *"the EF designation … refers to its **presumed GC-independent origin rather than its location**"* (see [[Eisenbarth2025 - A Roadmap for Defining Extrafollicular B Cell Responses]], consensus Perspective, no primary data). **No panel below changes.** What these tubes characterise is a **DN2/ABC population whose origin is inferred to be GC-independent**, plus its activation and ASC-differentiation state — read "EF effector," "EF migration" and "EF chemokine switch" in that sense throughout. The same source cautions that CD21ˡᵒ/CXCR5⁻/CD11c⁺ *"could indicate recent B cell activation rather than a permanent state"* — an argument for the internally-normalised **DN2:DN1 ratio** over raw frequencies, and for serial sampling. See [[GC-Independent Response]], [[DN2 Gating Strategy]].
...
- [[Woodruff2020 - EF B Cell Responses in COVID-19]] — DN1/DN2/DN3 resolution requires CXCR5; DN2:DN1 ratio as the cross-disease EF metric; Emory DN3 = CD11c⁻**CD21**⁻
- [[Allard-Chamard2023 - DN3 B Cells Infiltrate Inflamed Tissues]] — the DN1–DN4 primary; gates DN on **CXCR5 × CD11c with no CD21**, i.e. an axis orthogonal to the CD21-based scheme; contested DN4 assignment
- [[Eisenbarth2025 - A Roadmap for Defining Extrafollicular B Cell Responses]] — consensus Perspective: no flow panel alone establishes an EF response; claim GC-independence, not location
...
- [[Glaros2025 - Multilayered Identity of B Cell Memory]] — T-bet **not strictly required** for CD11c⁺ ABC formation (mouse conditional-KO); ZEB2 as the cross-context driver; CD11c stays the primary axis
...
- [[Cancro2020 - Age-Associated B Cells]] — quantitative prior for T-bet/CD11c heterogeneity within the ABC gate (F4); independent, earlier support for CD11c as cytokine-direct rather than T-bet-downstream (F2)
...
Also surfaces the **CXCR5⁺ DN subsets (DN1/DN4)** that a standard CXCR5⁻ EF gate discards (current watch item) — and note that **DN4's phenotype is contested**: the primary gates it CXCR5⁺**CD11c⁺** (see [[Allard-Chamard2023 - DN3 B Cells Infiltrate Inflamed Tissues]], Fig. 1B/1C, confirmed Fig. 6C), while [[Lamprinou2026 - ABCs and DN B Cells]] relays CXCR5⁺**CD11c⁻**, which would equal DN1. Unresolved; recorded on [[Double-Negative B Cell]]. *Option:* swap IgA→T-bet-APC (move CXCR3→BV711) to add IC confirmation here.
...
- **★ [2026-08-28] Panel 4 can settle the DN3 gating-axis conflict — the highest-value reconciliation currently in reach on this bench.** Emory/Sanz gate DN3 as IgD⁻CD27⁻**CD11c⁻CD21⁻** ([[Woodruff2020 - EF B Cell Responses in COVID-19]]); Pillai/Ragon gate it as IgD⁻CD27⁻**CXCR5⁻CD11c⁻** ([[Allard-Chamard2023 - DN3 B Cells Infiltrate Inflamed Tissues]], no CD21 in panel). **Neither published panel contains the other's discriminating marker, and no study has run both axes on the same sample** — so every cross-study DN3 comparison in this wiki rests on the untested assumption that CXCR5⁻ and CD21⁻ select the same cells within the CD11c⁻ DN pool. **Panel 4 carries CXCR5, CD21, CD11c and T-bet together and would produce the first direct answer.** A secondary payoff: it would bound how much DN4 (CXCR5⁺CD11c⁺, contested) contaminates a CD21-based DN2-phenotype gate — currently unbounded, since DN4's CD21 status has never been published. See [[DN2 Gating Strategy]], [[DN3 B Cell]].
- **★ [2026-08-16] Report T-bet as a fraction, not as a gate requirement — T-bet is dispensable for ABC formation.** CD11c⁺ ABCs arise in both infection and autoimmune models **in the absence of T-bet**, with [[ZEB2]] proposed as the shared cross-context driver instead (see [[Glaros2025 - Multilayered Identity of B Cell Memory]], review, citing two mouse conditional-KO primaries; corroborates [[Sanz2025 - Human Atypical B Cells Overview]] citing Yang 2022 / Du 2019). **Consequence for this panel:** the design above treats T-bet as the marker that converts "DN2-phenotype" → *confirmed* DN2/ABC. That framing should be softened — a **T-bet-gated DN2 call is a lower bound** that will miss a T-bet-independent CD11c⁺ fraction of unknown size in human acute infection. Recommended reporting: keep **CD11c/CD21/CXCR5 as the primary gate** (consistent with [[Sutton2021 - Alternative Lineage B Cells in Vaccination and Infection]], where CD11c was the best single surface marker) and report **T-bet⁺ as a percentage *within* that gate**. The concordance question above then becomes the measurement, rather than an assumption baked into the gate. See [[T-bet]] Contradictions.
- **★ [2026-08-16] Cancro2020 supplies the quantitative prior for that recommendation, plus independent support for treating CD11c as the primary axis.** Within the murine CD21⁻CD23⁻ splenic B cell pool, **only ~two-thirds are T-bet⁺**, and among those, **roughly half are CD11c⁺** — i.e., a T-bet-gated ABC/DN2 call is a lower bound of estimable magnitude (a CD21⁻-only gate over-calls by ~50%; a T-bet-only gate under-calls the CD21⁻CD23⁻ pool by ~a third) (see [[Cancro2020 - Age-Associated B Cells]], review — no original data; mouse, splenic). Separately, transcriptional analysis of IFN-γ/IL-21-treated WT vs T-bet-deficient cells found **CD11c expression is largely a direct cytokine effect rather than a T-bet-downstream target** — independent support (from a different evidence type than [[Glaros2025 - Multilayered Identity of B Cell Memory]]'s genetic KOs) for keeping **CD11c/CD21/CXCR5 as the primary gate** and reporting T-bet⁺ as a percentage within it, exactly as this page already recommends. **Caveat: both figures are murine and splenic** — whether the ~2/3 T-bet⁺ / ~half CD11c⁺ ratio transfers to human peripheral blood is untested, and is itself listed as an open question on the source page.
- **Worth considering: is ZEB2 the more informative intracellular slot than T-bet?** ZEB2 is now the leading candidate for a cross-context ABC identity factor in both mice and humans, it is upstream of T-bet, and **no dengue study has stained it** — so it would be novel rather than confirmatory. It is intranuclear, so it fits the same TF perm workflow. Against: no validated flow clone is documented in this wiki, T-bet has the direct Jenks2018/Woodruff2020 comparator data that ZEB2 lacks, and swapping would forfeit cross-study comparability. Flagged as an option, not a recommendation (see [[ZEB2]]).
