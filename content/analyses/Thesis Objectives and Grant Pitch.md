...
- **★ [2026-08-27] Say "GC-independent," not "extrafollicular" — a nomenclature constraint with teeth, from a consensus panel that includes Sanz.** [[Eisenbarth2025 - A Roadmap for Defining Extrafollicular B Cell Responses]] (*Immunity*, Nov 2025, 12 authors) states that *"there are no flow cytometry-based means alone that can distinguish EF B cells nor their progeny,"* that EF *"should be avoided unless proliferation of antigen-specific B cells outside of a follicle is observed,"* and — of the exact cell this pilot measures — that *"the EF designation of this human DN2 cell refers to its presumed GC-independent origin rather than its location."* **Nothing in the biology changes and no objective is at risk**: O1–O5 all rest on origin/process reasoning, which is untouched. What changes is the vocabulary of the claim. Write **"GC-independent (non-GC) B cell response"** and state the criteria; reserve "extrafollicular" for citing others' tissue-imaged work. Two live benefits: it pre-empts the reviewer who has read this Perspective, and it is the *stronger* rhetorical position — claiming only what the design supports. Note also the paper's second caution, that CD21ˡᵒ/CXCR5⁻/CD11c⁺ *"could indicate recent B cell activation rather than a permanent state"* — a further reason the DN2:DN1 **ratio** (internally normalised) is the better centrepiece than any raw frequency. See [[GC-Independent Response]].
...
- Should the thesis adopt the consensus **"non-GCB"** process-based vocabulary outright (aNAV → "primary non-GCB", DN2 → "primary switched non-GCB"), or keep DN1/DN2 with a stated definition? Held as an annotation layer for now (curator decision 2026-08-27); a thesis submitted in 2027 will be read against whatever the field has settled on.
